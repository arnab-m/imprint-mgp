...
class MyServer(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        # self.wfile.write(bytes("<html><head><title>Generated Model Control</title></head>", "utf-8"))
        # self.wfile.write(bytes("<p>Request: %s</p>" % self.path, "utf-8"))
        # self.wfile.write(bytes("<body>", "utf-8"))
        # self.wfile.write(bytes("<p>Controller has started working. Please wait. Results will be posted soon.</p>", "utf-8"))
        # self.wfile.write(bytes("</body></html>", "utf-8"))

        #Loading Data
        alpha, eta_c, eta_d, gamma_b = 10, .95, .98, 2
        ...
        DG_exists, Bat_exists = True, True
        P_PV_t = np.genfromtxt('PV_gen.csv', delimiter=',',skip_header=1).tolist()#[random.randint(1000,1500) for t in range(T)] #PV Generation
        P_WT_t = np.genfromtxt('WT_gen.csv', delimiter=',',skip_header=1).tolist()#[random.randint(1500,2500) for t in range(T)] #Wind Generation
        P_L_t = np.genfromtxt('Load.csv', delimiter=',',skip_header=1).tolist()#[random.randint(3000,5000) for t in range(T)]#Load
        ...
        if DG_exists:
            ...
            if Bat_exists:
                my_colnames = P_MG_b + P_MG_s + S_MG_b + S_MG_s + SP_MG_b + SP_MG_s
                my_colnames+= P_B_dch + P_B_ch + S_B_dch+ S_B_ch + SP_B_dch+ SP_B_ch + SOC
            else:
                my_colnames = P_MG_b + P_MG_s + S_MG_b + S_MG_s + SP_MG_b + SP_MG_s
        #---------------------------------------------------------
        ...
        if DG_exists:
            if Bat_exists:
                #Power Balance constraint, at every time slot t when both DG and Battery exists
                for t in range(T):
                ...
                for t in range(T):
                    ...
                    my_rhs += [P_L_t[t]-(P_PV_t[t]+P_WT_t[t])] #Load - (PV + WT generation)
                    my_sense +="G"
                    var = ["P_DG_"+str(t), "SP_MG_b_"+str(t), "SP_MG_s_"+str(t)]
                    coef = [1,1,-1,]
                    rows.append([var,coef])
            ...
            if Bat_exists:
                    ...
                    for t in range(T):
                        ...
                        my_rhs += [P_L_t[t]-(P_PV_t[t]+P_WT_t[t])] #Load - (PV + WT generation)
                        my_sense +="G"
                        var = ["SP_MG_b_"+str(t), "SP_MG_s_"+str(t), "SP_B_dch_"+str(t), "SP_B_ch_"+str(t)]
                        coef = [1,-1,1,-1]
                        rows.append([var,coef])
            else:
                #Power Balance constraint, at every time slot t when both DG and Battery exists
                for t in range(T):
                    ...
                    my_rhs += [P_L_t[t]-(P_PV_t[t]+P_WT_t[t])] #Load - (PV + WT generation)
                    my_sense +="G"
                    var = ["SP_MG_b_"+str(t), "SP_MG_s_"+str(t)]
                    coef = [1,-1,]
                    rows.append([var,coef]) 
        if DG_exists:
        ...
        for t in range(0, T):
            if Bat_exists:
                ...
                non_lin_vars += ["SP_MG_b_"+str(t), "SP_MG_s_"+str(t)]
                lin_vars += ["P_MG_b_"+str(t), "P_MG_s_"+str(t)]
                lin_vars_min += [P_MG_min, P_MG_min]
                lin_vars_max += [P_MG_max, P_MG_max]
                status_vars += ["S_MG_b_"+str(t), "S_MG_s_"+str(t)]
        ...
        if DG_exists:
            if Bat_exists:      
                ...
                #Plotting Data
                fig, ax = plt.subplots()
                l1, = ax.plot(range(1,T+1), P_PV_t, 'go--', linewidth=2, markersize=5)
                l2, = ax.plot(range(1,T+1), P_WT_t, 'gv-', linewidth=2, markersize=5)
                l3, = ax.plot(range(1,T+1), P_L_t, 'rs-.', linewidth=2, markersize=5)
                l4, = ax.plot(range(1,T+1), P_MG_b_values, 'cp-.', linewidth=2, markersize=5)
                l5, = ax.plot(range(1,T+1), P_MG_s_values, 'c^--', linewidth=2, markersize=5)
                l6, = ax.plot(range(1,T+1), P_B_dch_values, 'k>-.', linewidth=2, markersize=5)
                l7, = ax.plot(range(1,T+1), P_B_ch_values, 'kd--', linewidth=2, markersize=5)
                l8, = ax.plot(range(1,T+1), P_DG_values, 'b<--', linewidth=2, markersize=5)
                ax.set_ylabel("Power (in KiloWatts)")
                ax.set_xlabel("Hourly Time slots")
                fig.legend((l1,l2,l3,l4,l5,l6,l7,l8), ("PV", "Wind", "Load", "MG_buy", "MG_sell","Bat_dch", "Bat_ch","DG"), "upper right")
                plt.savefig("plot_with_bat_and_DG.png", dpi=300)
            else:
                my_colnames = P_DG + P_MG_b + P_MG_s + S_MG_b + S_MG_s + SP_MG_b + SP_MG_s
                P_DG_values = my_prob.solution.get_values(P_DG)
                P_MG_b_values = my_prob.solution.get_values(P_MG_b)
                ...
                with open("data.csv", "w") as csvfile:
                    writerobj = csv.writer(csvfile, delimiter=',')
                    writerobj.writerow(["TSlt", "PV_Gen", "WT_Gen", "Load", "DG_Gen", "MG_buy", "MG_sell"])
                    for t in range(T):
                        writerobj.writerow([t+1, P_PV_t[t], P_WT_t[t], P_L_t[t], round(P_DG_values[t]), round(P_MG_b_values[t]), round(P_MG_s_values[t])])
                    writerobj.writerow(["solution", my_prob.solution.get_objective_value()])
                #Plotting Data
                fig, ax = plt.subplots()
                l1, = ax.plot(range(1,T+1), P_PV_t, 'go--', linewidth=2, markersize=5)
                ...
                ax.set_ylabel("Power (in KiloWatts)")
                ax.set_xlabel("Hourly Time slots")
                fig.legend((l1,l2,l3,l4,l5,l8), ("PV", "Wind", "Load", "MG_buy", "MG_sell", "DG"), "upper right")
                plt.savefig("plot_with_DG.png", dpi=300)
            ...
            if Bat_exists:      
                ...
                with open("data.csv", "w") as csvfile:
                    writerobj = csv.writer(csvfile, delimiter=',')
                    writerobj.writerow(["TSlt", "PV_Gen", "WT_Gen", "Load", "MG_buy", "MG_sell", "Bat_dch", "Bat_ch", "SOC"])
                    for t in range(T):
                        writerobj.writerow([t+1, P_PV_t[t], P_WT_t[t], P_L_t[t], round(P_MG_b_values[t]), round(P_MG_s_values[t]), round(P_B_dch_values[t]), round(P_B_ch_values[t]), round(SOC_values[t])])
                    writerobj.writerow(["solution", my_prob.solution.get_objective_value()])
                #Plotting Data
                fig, ax = plt.subplots()
                l1, = ax.plot(range(1,T+1), P_PV_t, 'go--', linewidth=2, markersize=5)
                l2, = ax.plot(range(1,T+1), P_WT_t, 'bv-', linewidth=2, markersize=5)
                l3, = ax.plot(range(1,T+1), P_L_t, 'rs-.', linewidth=2, markersize=5)
                l4, = ax.plot(range(1,T+1), P_MG_b_values, 'cp-.', linewidth=2, markersize=5)
                l5, = ax.plot(range(1,T+1), P_MG_s_values, 'cp--', linewidth=2, markersize=5)
                l6, = ax.plot(range(1,T+1), P_B_dch_values, 'kd-.', linewidth=2, markersize=5)
                l7, = ax.plot(range(1,T+1), P_B_ch_values, 'kd--', linewidth=2, markersize=5)
                ax.set_ylabel("Power (in KiloWatts)")
                ax.set_xlabel("Hourly Time slots")
                fig.legend((l1,l2,l3,l4,l5,l6,l7), ("PV", "Wind", "Load", "MG_buy", "MG_sell","Bat_dch", "Bat_ch"), "upper right")
                plt.savefig("plot_with_bat.png", dpi=300)
            else:
                my_colnames = P_MG_b + P_MG_s + S_MG_b + S_MG_s + SP_MG_b + SP_MG_s
                P_MG_b_values = my_prob.solution.get_values(P_MG_b)
                P_MG_s_values = my_prob.solution.get_values(P_MG_s)
                ...
                with open("data.csv", "w") as csvfile:
                    writerobj = csv.writer(csvfile, delimiter=',')
                    writerobj.writerow(["TSlt", "PV_Gen", "WT_Gen", "Load", "MG_buy", "MG_sell"])
                    for t in range(T):
                        writerobj.writerow([t+1, P_PV_t[t], P_WT_t[t], P_L_t[t], round(P_MG_b_values[t]), round(P_MG_s_values[t])])
                    writerobj.writerow(["solution", my_prob.solution.get_objective_value()])
                #Plotting Data
                fig, ax = plt.subplots()
                l1, = ax.plot(range(1,T+1), P_PV_t, 'go--', linewidth=2, markersize=5)
                l2, = ax.plot(range(1,T+1), P_WT_t, 'bv-', linewidth=2, markersize=5)
                l3, = ax.plot(range(1,T+1), P_L_t, 'rs-.', linewidth=2, markersize=5)
                l4, = ax.plot(range(1,T+1), P_MG_b_values, 'yp-.', linewidth=2, markersize=5)
                l5, = ax.plot(range(1,T+1), P_MG_s_values, 'cp-.', linewidth=2, markersize=5)
                ax.set_ylabel("Power (in KiloWatts)")
                ax.set_xlabel("Hourly Time slots")
                fig.legend((l1,l2,l3,l4,l5), ("PV", "Wind", "Load", "MG_buy", "MG_sell"), "upper right")
